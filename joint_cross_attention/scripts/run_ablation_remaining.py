...
ABLATONS = [
    ('sg_aca_symmetric_attention', 'sg_aca_symmetric_attention.yaml'),
    ('no_amcf_concat', 'no_amcf_concat.yaml'),
    ('no_amcf_static_learned', 'no_amcf_static_learned.yaml'),
]


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run the remaining joint-model ablations sequentially')
    p.add_argument('--config', nargs='+', required=True)
    p.add_argument('--override', nargs='*', default=[])
    p.add_argument('--output', default='joint_cross_attention/outputs/cache/joint_model_ablation_remaining_report.json')
...
def _write_csv(path: Path, runs: list[dict]) -> None:
    ...
    with path.open('w', newline='', encoding='utf-8') as f:
        ...
        writer.writeheader()
        for row in runs:
            summary = row.get('summary_metrics', {})
            writer.writerow(
                {
                    'name': row.get('name', ''),
...
def main() -> None:
    ...
    for name, cfg_name in ABLATONS:
        ...
        print(f'[ablation] running {name}: {' '.join(cmd)}')
        env = dict(os.environ)
