...
PLAN = [
    ('paper_baseline', 'paper_baseline.yaml'),
    ('paper_variant1_acg', 'paper_variant1_acg.yaml'),
    ('paper_variant2_acg_csip', 'paper_variant2_acg_csip.yaml'),
    ('paper_variant3_mdm', 'paper_variant3_mdm.yaml'),
    ('paper_full', 'paper_full.yaml'),
]


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run the TC-SVT paper ablations for the joint model sequentially')
    p.add_argument('--config', nargs='+', required=True)
    p.add_argument('--override', nargs='*', default=[])
    p.add_argument('--output', default='joint_cross_attention/outputs/cache/paper_tcsvt_ablation_report.json')
    p.add_argument('--resume', action=argparse.BooleanOptionalAction, default=True)
    p.add_argument('--include-full', action=argparse.BooleanOptionalAction, default=True)
...
def _write_csv(path: Path, rows: list[dict]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    headers = ['name', 'returncode', 'run_dir', 'monitor', 'best_metric', 'acc', 'f1', 'precision', 'recall', 'threshold', 'best_checkpoint']
...
def main() -> None:
    ...
    cfg_dir = root / 'joint_cross_attention' / 'configs' / 'paper_ablation'
    ...
    if not args.include_full:
        plan = [item for item in plan if item[0] != 'paper_full']

    report = _load_existing(out) if args.resume else None
    if report is None:
        report = {'created_at': datetime.now().isoformat(timespec='seconds'), 'runs': []}
    report['include_full'] = bool(args.include_full)
    rows = report.get('runs', []) if isinstance(report.get('runs'), list) else []
    done = {
        str(r.get('name'))
        for r in rows
        if isinstance(r, dict) and int(r.get('returncode', 1)) == 0 and str(r.get('run_dir', '')).strip()
    }

    for name, cfg_name in plan:
        if args.resume and name in done:
            print(f'[paper-ablation] skip completed: {name}')
            continue

        cfg_path = cfg_dir / cfg_name
        merged_overrides = list(args.override)
        ...
        print(f"[paper-ablation] running {name}: {' '.join(cmd)}")
        proc = subprocess.run(cmd, cwd=str(root), check=False, env=dict(os.environ))
